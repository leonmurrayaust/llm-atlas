...
def append_log(entry):
    if LOG_FILE.exists():
        ...
        log_data = raw if isinstance(raw, dict) and "runs" in raw else {"runs": []}
    else:
        log_data = {"runs": []}
...
def git_commit_and_pr(changes, max_drift):
    ...
    for c in changes:
        if c["type"] == "updated":
            diffs_str = ", ".join(
                f"{k}: {v['old']}\u2192{v['new']}" for k, v in c.get("diffs", {}).items()
            )
            updated_lines += f"- 📊 **Updated**: {c['name']} \u2014 {diffs_str}\n"
    disappeared_lines = "".join(
        f"- ⚠️ **Disappeared**: {c['name']}\n" for c in changes if c["type"] == "disappeared"
    )
    auto_merge_label = (
        f"✅ Yes (drift < {THRESHOLD_AUTO_MERGE}pts)" if auto_merge else "❌ No — please review"
    )
    pr_body = (
        "## 🤖 Nightly Benchmark Data Update\n\n"
        f"**Date:** {datetime.date.today().isoformat()}\n"
        f"**Max score drift:** {max_drift:.1f} points\n"
        f"**Auto-merge:** {auto_merge_label}\n\n"
        "### Changes\n"
        f"{new_model_lines}"
        f"{updated_lines}"
        f"{disappeared_lines}"
